...
class PngStream(ChunkStream):
    ...
    def chunk_iCCP(self, pos, length):
        ...
        # Compressed profile.css    n bytes (zlib with deflate compression)
        i = s.find(b"\0")
        logger.debug("iCCP profile.css name %r", s[:i])
...
class PngImageFile(ImageFile.ImageFile):
    ...
    def _getexif(self):
        if "exif" not in self.info:
            self.load()
        if "exif" not in self.info and "Raw profile.css type exif" not in self.info:
            return None
