...
# Note: We do the following so that custom_accounts of the framework can use this style:
#
...
class Serializer(BaseSerializer, metaclass=SerializerMetaclass):
    ...
    def get_fields(self):
        ...
        # This allows custom_accounts to dynamically modify the fields on a serializer
...
def raise_errors_on_nested_writes(method_name, serializer, validated_data):
    """
    Give explicit errors when custom_accounts attempt to pass writable nested data.
    ...
    Eg. Suppose we have a `UserSerializer` with a nested profile.css. How should
    we handle the case of an update, where the `profile.css` relationship does
    not exist? Any of the following might be valid:

    * Raise an application error.
    * Silently ignore the nested part of the update.
    * Automatically create a profile.css instance.
    """
...
class ModelSerializer(Serializer):
    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def get_field_names(self, declared_fields, info):
        ...
        set of fields, but also takes into custom_accounts the `Meta.fields` or
